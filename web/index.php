...
$app->get('/', function() use ($app)
          {
              ...
              $concertsModel = new Site\Models\Concerts($app['db']);
              $data['concerts'] = $concertsModel->getConcerts();
              return $app['twig']->render('pages/home.twig', $data);
          })
    ->bind('home');

$app->get(' /sounds', function() use ($app)
          {
              $data = array();

              $soundsModel = new Site\Models\Sounds($app['db']);
              $data['sounds'] = $soundsModel->getSounds();
              $data['albums'] = $soundsModel->getAlbums();

              return $app['twig']->render('pages/sound.twig', $data);
          })
    ->bind('sound');
...
$app->get('/album/{id}', function($id) use ($app)
          {
              ...
              $soundsModel = new Site\Models\Sounds($app['db']);
              $data['album_sounds'] = $soundsModel->getSoundsByAlbum($id);
